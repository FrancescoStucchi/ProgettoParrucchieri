...
<body>
    <div id="header">
        <div>
            ...
            <% 
                String nomeSede = "";
                String id_sede;
                Gestore gestore = null; // Dichiarazione della variabile gestore
                try {
                    gestore = new Gestore(); // Inizializzazione della variabile gestore
                    ...
                    // Settiamo la pagina precedente
                    session.setAttribute("paginaPrecedente", "visualizzazioneAppuntamentiGlobali.jsp");
    ...
    <div class="container">
        <div class="date-navigation">
            <%
                // Get the current date or the date parameter from the request
            ...
            %>
            <button onclick="window.location.href='?date=<%= previousDate %>'">Previous Day</button>
            <button onclick="window.location.href='?date=<%= nextDate %>'">Next Day</button>
        </div>
                ...
                <%
                    try {
                        String sql = "SELECT appuntamenti.id, appuntamenti.data, appuntamenti.durata, appuntamenti.ora, " +
                                     ...
                                     "WHERE appuntamenti.data = '" + Date.valueOf(currentDate) + "'";
                        ResultSet rs = gestore.getFunzioni().select(sql);
                        while (rs.next()) {
                            // Retrieve the start time and duration
